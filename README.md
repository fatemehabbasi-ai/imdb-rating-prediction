...
- The real limitation is the features, not the model: these four columns alone explain about 64% of the variation in rating. Getting a meaningfully better model would likely require better features (genre, director, cast, runtime), not a more complex algorithm.
...
- Comparing a linear model (Linear Regression) with a non-linear one (Random Forest) using the exact same train/test split